...
export function createReactive(origin, options = {}) {
  ...
  const createObject = (origin, parents = []) => {
    ...
    const put = (key, value, trigger) => {
      ...
      const setValue = (value, prev, trigger) => {
        const input = isFunction(set) ? set(keyPath, value) : value
        // if input === prev, it means not change
        const next = input === prev ? input : create(input, keyPath)

        origin[key] = input
        media[key] = next

        if (trigger && isFunction(dispatch)) {
          dispatch({ keyPath, value, input, next, prev })
        }
      ...
      Object.defineProperty(reactive, key, {
        ...
        set: (value) => {
          const prev = media[key]

          if (isFunction(writable) && !writable(keyPath)) {
            return prev
          }

          const next = setValue(value, prev, true)
          return next
        },
        enumerable: true,
        configurable: true,
      ...
      $del: {
        value: (key) => {
          ...
          if (isFunction(dispatch)) {
            dispatch({ keyPath, value: undefined, input: undefined, next: undefined, prev })
          }
  ...
  const createArray = (origin, parents = []) => {
    ...
    const modify = (fn) => ({
      value: function(...args) {
        ...
        if (isFunction(dispatch)) {
          dispatch({ keyPath: parents, value: origin, input: reactive, next: reactive, prev: reactive }, true)
        }
      ...
      get: (target, key, receiver) => {
        ...
        if (inArray(key, methods)) {
          return (...args) => {
            ...
            if (isFunction(dispatch)) {
              dispatch({ keyPath: parents, value: origin, input: proxy, next: proxy, prev: proxy }, true)
            }
